...
export default function Home() {
  ...
  return <main>
    <header className="site-header">
      ...
      <a className="header-cta" href="#contact">发起合作 <ArrowIcon /></a>
    ...
    <section className="works section" id="works">
      ...
      <div className="project-grid">{filtered.map((p, index) => <article className={`project-card ${index < 3 && category === "全部" ? "top" : ""}`} key={p.id}>
        ...
        <div className="card-content"><div className="card-title-row"><div><span>{String(p.id).padStart(2,"0")}</span><h3>{p.title}</h3></div>{p.featured && <b className="featured">精选</b>}</div>
          ...
          <div className="card-actions"><button onClick={() => setSelected(p)}>查看项目详情</button><button className="scheme" onClick={() => startContact(p)}>获取同类方案 <ArrowIcon /></button></div>
        </div>
      </article>)}</div>
    ...
    <section className="about section" id="about"><div className="about-card"><div className="about-copy"><p className="section-no">04 / ABOUT</p><h2>懂技术，更懂产品如何<br />创造真实业务价值</h2><p>我是一名 AI 应用架构师与企业级平台架构师，长期深耕金融科技、工业互联网、物联网、智慧城市与行业数字化。擅长把复杂技术能力转化为用户看得懂、业务用得上、团队接得住的数字产品。</p><div className="about-tags"><span>Java / Python</span><span>AI Agent</span><span>RAG / MCP</span><span>微服务架构</span><span>IoT 云边协同</span></div></div><div className="about-side"><div className="about-quote"><span>“</span><blockquote>好的数字产品，不是功能的堆砌，而是让复杂业务变得清晰、可信且高效。</blockquote><small>PRODUCT PHILOSOPHY</small></div><div className="wechat-card"><img src="/wechat-qr.jpg" alt="奋斗人生微信二维码" /><div><strong>微信扫码联系</strong><span>添加时请备注：项目合作</span></div></div></div></div></section>

    <section className="contact section" id="contact"><div className="contact-panel"><div><p className="section-no">START A PROJECT</p><h2>有一个想法？<br /><em>让我们把它做成产品。</em></h2><p>无论是新产品从 0 到 1、现有系统升级，还是 AI 能力融入业务，都可以先从一次需求沟通开始。</p></div><form onSubmit={(e) => { e.preventDefault(); alert("需求已记录。正式推广前可在此接入您的微信、电话或在线表单。"); }}><label><span>怎么称呼您</span><input required placeholder="请输入姓名 / 公司名称" /></label><label><span>联系方式</span><input required placeholder="微信 / 手机号" /></label><label><span>项目需求</span><textarea required defaultValue={typeof window !== "undefined" ? sessionStorage.getItem("interest") ? `我想了解：${sessionStorage.getItem("interest")}` : "" : ""} placeholder="简单描述您想做的产品或需要解决的问题" /></label><button className="primary-btn" type="submit">提交合作需求 <ArrowIcon /></button><small>提交即表示同意仅将信息用于本次项目沟通</small></form></div></section>

    <footer><a className="brand footer-brand" href="#top"><span className="brand-mark">CG</span><span><b>数字产品实验室</b><small>DIGITAL PRODUCT STUDIO</small></span></a><p>专注企业数字化产品、AI应用与产业互联网解决方案</p><div><a href="#works">作品</a><a href="#capability">能力</a><a href="#process">流程</a><a href="#contact">合作</a></div><small>© 2026 CAO GUOJUN. ALL RIGHTS RESERVED.</small></footer>

    {selected && <div className="modal-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && setSelected(null)}><section className="project-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><button className="modal-close" aria-label="关闭详情" onClick={() => setSelected(null)}>×</button><ProjectVisual project={selected} /><div className="modal-body"><p className="modal-kicker">PROJECT CASE · {String(selected.id).padStart(2,"0")}</p><h2 id="modal-title">{selected.title}</h2><p className="modal-overview">{selected.overview}</p><div className="modal-metrics">{selected.metrics.map(([v,l]) => <div key={l}><b>{v}</b><span>{l}</span></div>)}</div><h3>核心能力与交付范围</h3><ul className="highlight-list">{selected.highlights.map(x => <li key={x}><span><CheckIcon /></span>{x}</li>)}</ul><div className="modal-footer"><div><small>参考投入</small><b>{selected.price}</b><span> · {selected.cycle}</span></div><button className="primary-btn" onClick={() => startContact(selected)}>获取同类方案 <ArrowIcon /></button></div></div></section></div>}
  </main>;
}
